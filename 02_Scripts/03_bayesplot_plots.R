...
setwd("C:/Users/mmend/OneDrive/Documentos/ecoevolab")
# Load the data (stan outputs) ---------------------------------------------------
stan_toutputs <- readRDS("bacterial_comms/03_Output/stan_rk_vectors_slope_monod")
...
# plot()----------------------------------------------
pdf("bacterial_comms_output/traceplot_monod_slope.pdf", width = 15, height = 7)
...
pdf("bacterial_comms_output/mcmcpairs_monod_slope.pdf", width = 15, height = 7, onefile = TRUE)
...
for (i in 1:length(stan_toutputs)){
  ...
  x <- mcmc_pairs(pst_cp2, pars = c("r", "k"),
                  ...
                  condition = pairs_condition(chains = list(1, 2)), 
