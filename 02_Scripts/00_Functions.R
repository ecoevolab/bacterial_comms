...
stan_monodf <- function (df, temp_col, replica_col, strain_col, sample_byh, interest_col, inits_list, niterations, nchains, sigma_val, model){
  library(tidyverse)
  library(rstan)
  ...
  for (m in 1:length(spps)) {
    for (o in 1:length(ntemps)) {
      ...
      mumaxv <- row_id$r_init # for mumax value 
      
      if (mumaxv < 1.0) {
        ks_prior_center <- 0.02 
        sd_k <- 0.4   
        sd_r <- 0.3

      } else {
        ks_prior_center <- 0.10
        sd_k <- 1.0 
        sd_r <- 1.5
      ...
      yieldv <- mean(sapply(replica_list, function(sub_df) {
        max(sub_df[[interest_col]]) - min(sub_df[[interest_col]])
      }))
